...
--if campon == "true" then
	...
	freeswitch.consoleLog("notice", "Outgoing_evento campon no atendimento.: ".. tostring(uuid).." "  .. tostring(campon) .. "\n");
	freeswitch.consoleLog("notice", "Outgoing_evento call_uuid no atendimento.: " .. tostring(uuid).." " .. tostring(call_uuid) .. "\n");
	freeswitch.consoleLog("notice", "Outgoing_evento uuid no atendimento.: " .. tostring(uuid) .. "\n");
	freeswitch.consoleLog("notice", "Outgoing_evento destination_number no atendimento.: " .. tostring(uuid) .." " .. tostring(destination_number) .. "\n");
	freeswitch.consoleLog("notice", "Outgoing_evento variavel signal_bond no atendimento.: " .. tostring(uuid) .." " .. tostring(signal_bond) .. "\n");
	freeswitch.consoleLog("notice", "Outgoing_evento variavel endpoint_disposition no atendimento.: ".. tostring(uuid) .." " .. tostring(endpoint_disposition) .. "\n");

	freeswitch.consoleLog("notice", "Outgoing_evento " .. tostring(destination_number) .. " " .. tostring(uuid).."\n");
--if string.find(destination_number, "6725") then
--        dump = api:execute("uuid_dump", uuid)

        --if session:ready() then
--                freeswitch.consoleLog("notice", "Outgoing_evento ".. tostring(destination_number) .." " .. tostring(dump));
